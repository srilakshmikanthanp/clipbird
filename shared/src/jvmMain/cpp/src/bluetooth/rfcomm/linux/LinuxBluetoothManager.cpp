...
std::unique_ptr<io::Server> LinuxBluetoothManager::start(const std::string& serviceName, const boost::uuids::uuid& serviceUuid) {
  return std::make_unique<LinuxRfcommServer>(serviceName, serviceUuid);
}

std::unique_ptr<io::Channel> LinuxBluetoothManager::connect(const std::string& address, const boost::uuids::uuid& serviceUuid) {

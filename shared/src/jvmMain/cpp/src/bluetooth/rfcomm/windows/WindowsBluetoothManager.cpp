...
std::unique_ptr<io::Server> WindowsBluetoothManager::start(const std::string& serviceName, const boost::uuids::uuid& serviceUuid) {
  throw std::runtime_error("WindowsBluetoothManager::start() is not implemented yet.");
}

std::unique_ptr<io::Channel> WindowsBluetoothManager::connect(const std::string& address, const boost::uuids::uuid& serviceUuid) {

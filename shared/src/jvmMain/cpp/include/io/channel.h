...
typedef struct clipbird_io_channel clipbird_io_channel_t;
 ...
 */
bool clipbird_io_channel_read_exactly(clipbird_io_channel_t* channel, uint8_t* buffer, size_t length);
 ...
 * @return true on success, or false on failure. In case of failure, the last error can be retrieved using the error handling functions.
 */
bool clipbird_io_channel_write(clipbird_io_channel_t* channel, const uint8_t* data, size_t length);
